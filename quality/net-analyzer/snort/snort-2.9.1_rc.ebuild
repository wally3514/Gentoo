...
src_prepare() {
	...
	sed -i "s|${exec_prefix}/lib|${exec_prefix}/'$(get_libdir)'|g" \
...
pkg_config() {
	...
	select c_u in "Create" "Upgrade" "Exit"; do
		case ${c_u} in
			...
			sed -i "s|/usr/local/lib/snort_dynamicpreprocessor/|/usr/'$(get_libdir)'/snort_dynamicpreprocessor|g" \
				"${ROOT}etc/snort/${c_name}/snort.conf" || die "Failed to update snort.conf dynamicpreprocessor"
			sed -i "s|/usr/local/lib/snort_dynamicengine|/usr/'$(get_libdir)'/snort_dynamicengine|g" \
				"${ROOT}etc/snort/${c_name}/snort.conf" || die "Failed to update snort.conf dynamicengine"
			sed -i "s|/usr/local/lib/snort_dynamicrules|/usr/'$(get_libdir)'/snort_dynamicrules|g" \
			...
			sed -i "s|^preprocessor normalize_|# preprocessor normalize_|g" \
			...
			# Standard Changes
								# Set the correct rule location in the config
								sed -i "s|RULE_PATH ../rules|RULE_PATH /etc/snort/'${u_name}'/rules|g" \
									"${ROOT}etc/snort/${u_name}/snort.conf" || die "Failed to update snort.conf rule path"

								# Set the correct so_rule location in the config
								sed -i "s|SO_RULE_PATH ../so_rules|SO_RULE_PATH /etc/snort/'${u_name}'/so_rules|g" \
									"${ROOT}etc/snort/${u_name}/snort.conf" || die "Failed to update snort.conf so_rule path"

								# Set the correct preprocessor/decoder rule location in the config
								sed -i "s|PREPROC_RULE_PATH ../preproc_rules|PREPROC_RULE_PATH /etc/snort/'${u_name}'/preproc_rules|g" \
								...
								# Set the default log dir
								sed -i "s|^# config logdir:|config logdir: /var/log/snort/'${u_name}'|g" \
			...
			# Migrated Changes
								...
								if grep -q "^ *config daq:" ${old_conf}; then
									...
									sed -i "s|^# config daq:.*$|'${current_daq}'|g" \
								...
								if grep -q "^ *config daq_mode:" ${old_conf}; then
									...
									sed -i "s|^# config daq_mode:.*$|'${curent_daq_mode}'|g" \
								...
								if grep -q "^ *config daq_var:" ${old_conf}; then
									...
									sed -i "s|^# config daq_var:.*$|'${curent_daq_var}'|g" \
								...
								if grep -q "^ *config set_gid:" ${old_conf}; then
									...
									sed -i "s|^# config set_gid:.*$|'${current_gid}'|g" \
								...
								if grep -q "^ *config set_uid:" ${old_conf}; then
									...
									sed -i "s|^# config set_uid:.*$|'${current_uid}'|g" \
								...
								if grep -q "^ *config snaplen:" ${old_conf}; then
									...
									sed -i "s|^# config snaplen:.*$|'${current_snaplen}'|g" \
								...
								if grep -q "^ *config pcre_match_limit:" ${old_conf}; then
									...
									sed -i "s|^# config pcre_match_limit:.*$|'${current_pcreml}'|g" \
								...
								if grep -q "^ *config pcre_match_limit_recursion:" ${old_conf}; then
									...
									sed -i "s|^# config pcre_match_limit_recursion:.*$|'${current_pcremlr}'|g" \
								...
								if grep -q "^ *config checksum_mode:" ${old_conf}; then
									...
									sed -i "s|^config checksum_mode:.*$|'${current_checksum}'|g" \
								...
								if grep -q "^ *config response:" ${old_conf}; then
									...
									sed -i "s|^# config response:.*$|'${current_response}'|g" \
								...
								if grep -q "^ *config bpf_file:" ${old_conf}; then
									...
									sed -i "s|^# config bpf_file:.*$|'${current_bpf}'|g" \
								...
								if grep -q "^ *config detection:" ${old_conf}; then
									...
									sed -i "s|^config detection:.*$|'${current_detection}'|g" \
								...
								if grep -q "^ *config event_queue:" ${old_conf}; then
									...
									sed -i "s|^config event_queue:.*$|'${current_event}'|g" \
								...
								if grep -q "^ *config normalize_ip4" ${old_conf}; then
									...
									sed -i "s|^config normalize_ip4.*$|'${current_nip4}'|g" \
								...
								if grep -q "^ *config normalize_tcp" ${old_conf}; then
									...
									sed -i "s|^config normalize_tcp.*$|'${current_ntcp}'|g" \
								...
								if grep -q "^ *config normalize_icmp4" ${old_conf}; then
									...
									sed -i "s|^config normalize_icmp4.*$|'${current_nicmp4}'|g" \
								...
								if grep -q "^ *config normalize_ip6" ${old_conf}; then
									...
									sed -i "s|^config normalize_ip6.*$|'${current_nip6}'|g" \
								...
								if grep -q "^ *config normalize_icmp6" ${old_conf}; then
									...
									sed -i "s|^config normalize_icmp6.*$|'${current_nicmp4}'|g" \
								...
								if grep -q "^ *ipvar HOME_NET" ${old_conf}; then
									...
									sed -i "s|^ipvar HOME_NET.*$|'${current_home}'|g" \
								...
								if grep -q "^ *ipvar EXTERNAL_NET" ${old_conf}; then
									...
									sed -i "s|^ipvar EXTERNAL_NET.*$|'${current_external}'|g" \
								...
								if grep -q "^ *ipvar DNS_SERVERS" ${old_conf}; then
									...
									sed -i "s|^ipvar DNS_SERVERS.*$|'${current_dns}'|g" \
								...
								if grep -q "^ *ipvar SMTP_SERVERS" ${old_conf}; then
									...
									sed -i "s|^ipvar SMTP_SERVERS.*$|'${current_smtp}'|g" \
								...
								if grep -q "^ *ipvar HTTP_SERVERS" ${old_conf}; then
									...
									sed -i "s|^ipvar HTTP_SERVERS.*$|'${current_http}'|g" \
								...
								if grep -q "^ *ipvar SQL_SERVERS" ${old_conf}; then
									...
									sed -i "s|^ipvar SQL_SERVERS.*$|'${current_sql}'|g" \
								...
								if grep -q "^ *ipvar TELNET_SERVERS" ${old_conf}; then
									...
									sed -i "s|^ipvar TELNET_SERVERS.*$|'${current_telnet}'|g" \
								...
								if grep -q "^ *ipvar SSH_SERVERS" ${old_conf}; then
									...
									sed -i "s|^ipvar SSH_SERVERS.*$|'${current_ssh}'|g" \
								...
								if grep -q "^ *ipvar FTP_SERVERS" ${old_conf}; then
									...
									sed -i "s|^ipvar FTP_SERVERS.*$|'${current_ftp}'|g" \
								...
								if grep -q "^ *ipvar SIP_SERVERS" ${old_conf}; then
									...
									sed -i "s|^ipvar SIP_SERVERS.*$|'${current_sip}'|g" \
